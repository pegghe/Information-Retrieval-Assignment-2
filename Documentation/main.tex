...
\title{
    ...
    {\huge\bfseries\textcolor{uablue}{Wikipedia Movies Retrieval}}\\[0.5cm]
    {\Large High-Dimensional Similarity Search and Indexing Techniques}\\[0.3cm]
...
\date{
    ...
    \href{https://github.com/pegghe/Information-Retrieval-Assignment-2}{\textcolor{uablue}{https://github.com/pegghe/Information-Retrieval-Assignment-2}}
}
...
and Locality Sensitive Hashing (LSH) and compared them against FAISS, a production level 
...
Following the evaluation framework from Manning et al.~[1] (Chapter 8), we assess 
retrieval quality using \textbf{Recall@k}, \textbf{Precision@k}, \textbf{F1@k}, and \textbf{nDCG@k}.
We evaluate recall--efficiency trade-offs, scalability with respect to both the number 
...
\textbf{Key findings:}
\begin{itemize}
    \item \textbf{VQ} achieves the best balance of recall and efficiency among custom implementations, 
    ...
    \item \textbf{PQ} provides strong memory compression (up to 200$\times$) but sacrifices recall 
    significantly (max Recall@10 = 33\%).
    \item \textbf{FAISS-IVF} matches our VQ recall while being $\sim$15--40$\times$ faster due to 
...
We represent documents using \textbf{Word2Vec} embeddings~[6], for the following reasons:
...
Following the standard evaluation framework from Manning et al.~[1, Ch.~8] 
and the course slides on evaluation and feedback expansion~[2], 
we employ the following metrics...


\subsection{Metrics for Ranked Retrieval}

\subsubsection{Precision@k and Recall@k}

For a ranked list of retrieved documents:
\begin{align}
\text{Precision@}k &= \frac{|\{\text{relevant docs in top-}k\}|}{k} \\
\text{Recall@}k &= \frac{|\{\text{relevant docs in top-}k\}|}{|\{\text{all relevant docs}\}|}
\end{align}

\textbf{Note on ANN evaluation:} In approximate nearest neighbour search, we define 
``relevant documents'' as the $k$ exact nearest neighbours (ground truth from brute-force search).
Since we retrieve exactly $k$ documents and there are exactly $k$ relevant documents, 
\textbf{Precision@k = Recall@k} in our experiments.
...
When $P@k = R@k$ (as in our ANN context), $F_1@k = P@k = R@k$.

\subsubsection{Normalized Discounted Cumulative Gain (nDCG@k)}

Measures ranking quality by rewarding relevant documents appearing at higher positions:
\[
\text{DCG@}k = \sum_{i=1}^{k} \frac{\text{rel}_i}{\log_2(i+1)}, \qquad
\text{nDCG@}k = \frac{\text{DCG@}k}{\text{IDCG@}k}
\]

where $\text{rel}_i = 1$ if document at position $i$ is in the true top-$k$, and 
IDCG is the ideal DCG achieved by perfect ranking.

\subsection{Efficiency Metrics}

\subsubsection{Candidate Ratio}
The fraction of the corpus examined before returning results:
\[
\text{Candidate Ratio} = \frac{|\text{candidates examined}|}{N}
\]

Lower values indicate better efficiency.

\subsubsection{Query Time}
Wall-clock time per query, averaged over multiple queries.

\subsubsection{Build Time}
Time to construct the index from the document collection.

\subsection{Why Not Classical Accuracy?}

Following Manning et al., we \textbf{do not use accuracy} ($\frac{TP + TN}{TP + TN + FP + FN}$) 
because it is dominated by true negatives in retrieval tasks. A system that returns nothing 
achieves $\sim$99.99\% accuracy on a corpus of 17,830 documents when retrieving 10 clearly 
a useless metric for IR evaluation.
...
\begin{itemize}
    ...
    \item The recall--efficiency trade-off is smooth: doubling $n_{\text{probe}}$ roughly halves the error.
...
For cosine similarity, we use \textbf{random hyperplane hashing}~[8]:
...
\textbf{Key observations:}
\begin{itemize}
    \item LSH can achieve perfect recall (100\%) with a dense band configuration ($b=16$, $r=8$), 
but this effectively degenerates into an almost exhaustive search (candidate ratio $\approx 98\%$).
...
Product Quantization (PQ) compresses vectors by~[7]:
\begin{enumerate}
...
\textbf{Key observations:}
\begin{itemize}
    \item PQ achieves significant compression (50--200$\times$) but at substantial recall cost.
...
FAISS (Facebook AI Similarity Search) is an open-source library developed by Meta AI Research for efficient similarity search and clustering of dense vectors~[9,12].
...
\begin{itemize}
    ...
    \item Provides memory--recall trade-off
\end{itemize}
...
\begin{itemize}
    ...
    \item FAISS-IVFPQ underperforms due to the same PQ recall limitations.
...
Following Manning et al.~[1], Chapter 8:
\begin{itemize}
    \item 50 random query documents per configuration
    \item Ground truth: exact brute-force search (FAISS IndexFlatL2)
    \item \textbf{Metrics:} Recall@10, Precision@10, F1@10, nDCG@10, candidate ratio, query time
    \item Note: In ANN context, Precision@k = Recall@k since $|\text{retrieved}| = |\text{relevant}| = k$
    \item All experiments use \texttt{np.random.seed(42)} for reproducibility
\end{itemize}

\subsection{Experiment 1: Recall vs.\ Efficiency Trade-off}

We compare VQ, PQ, LSH, and FAISS-IVF on the full dataset ($N = 17{,}830$, $d = 200$).

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/exp1_recall_vs_candidate_ratio.png}
        \caption{Recall@10 vs.\ Candidate Ratio}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/exp1_ndcg_vs_candidate_ratio.png}
        \caption{nDCG@10 vs.\ Candidate Ratio}
    \end{subfigure}
    \caption{Effectiveness vs.\ efficiency trade-off.}
    \label{fig:exp1-efficiency}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/exp1_recall_vs_query_time.png}
    \caption{Recall@10 vs.\ Query Time (log scale). FAISS-IVF achieves similar recall 15--40$\times$ faster.}
    \label{fig:exp1-speed}
\end{figure}

\textbf{Analysis:}
\begin{itemize}
    \item \textbf{VQ} offers the best recall--efficiency trade-off among custom implementations.
    \item \textbf{LSH} can reach 100\% recall but requires examining most of the corpus.
    \item \textbf{PQ} cannot exceed 33\% recall inadequate for retrieval.
    \item \textbf{FAISS-IVF} matches VQ recall with microsecond-level query times.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/exp2_build_time_vs_N.png}
        \caption{Index construction time}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/exp2_query_time_vs_N.png}
        \caption{Query time (log scale)}
    \end{subfigure}
    \caption{Scaling behaviour with dataset size $N$.}
    \label{fig:scaling-n}
\end{figure}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/exp3_build_time_vs_dim.png}
        \caption{Index construction time}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/exp3_query_time_vs_dim.png}
        \caption{Query time (log scale)}
    \end{subfigure}
    \caption{Scaling behaviour with embedding dimensionality $d$.}
...
Figures~\ref{fig:summary-effectiveness} and~\ref{fig:summary-timing} provide an overview of best-case performance.

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/summary_best_recall.png}
        \caption{Best Recall@10}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/summary_best_ndcg.png}
        \caption{Best nDCG@10}
    \end{subfigure}
    \caption{Retrieval effectiveness comparison (best configuration per method).}
    \label{fig:summary-effectiveness}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/summary_build_time.png}
        \caption{Index build time at $N$=10K}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{images/summary_query_time.png}
        \caption{Query time (log scale)}
    \end{subfigure}
    \caption{Timing comparison across methods.}
    \label{fig:summary-timing}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/pr_ndcg_vs_recall.png}
    \caption{Ranking quality (nDCG@10) vs.\ retrieval quality (Recall@10). 
    Points near the diagonal indicate good ranking of retrieved documents.}
    \label{fig:ndcg-vs-recall}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{FAISS is consistently the fastest option in our experiments.}
    FAISS-IVF achieves similar recall to our VQ while being 15--40$\times$ faster,
...
\begin{itemize}
    \item \textbf{Use VQ/IVF} when high recall is required and memory is not the bottleneck.
    ...
    often one to two orders of magnitude faster in our setup.
\end{itemize}
...
\noindent
Manning, C. D., Raghavan, P., \& Sch\"{u}tze, H. (2008).
\textit{Introduction to Information Retrieval}. Cambridge University Press.\\
Available at: \url{https://nlp.stanford.edu/IR-book/pdf/irbookonlinereading.pdf}

\medskip

\noindent
Calders, T., \& Tokpo, E. (n.d.).
\textit{03b – Evaluation and Feedback Expansion}. Lecture slides, University of Antwerp,
...
\noindent
Calders, T. (n.d.).
\textit{04b – Dimensionality Reduction and Latent Semantic Indexing}. Lecture slides,
University of Antwerp, Information Retrieval.

\medskip

\noindent
Calders, T. (n.d.).
\textit{06 – Noncontextual Word Representations}. Lecture slides,
University of Antwerp, Information Retrieval.
...
\noindent
Calders, T. (n.d.).
\textit{07 – Querying Highdimensional Spaces}. Lecture slides,
University of Antwerp, Information Retrieval.
...
In \textit{Advances in Neural Information Processing Systems 26 (NeurIPS 2013)}.\\
Preprint: \url{https://arxiv.org/abs/1310.4546}
...
\textit{IEEE Transactions on Pattern Analysis and Machine Intelligence}, 33(1), 117128.
...
In \textit{Proceedings of the 34th Annual ACM Symposium on Theory of Computing (STOC '02)}
(pp.\ 380388).
...
\textit{IEEE Transactions on Big Data}, 7(3), 535547.
...
\textit{Journal of Machine Learning Research}, 12, 28252830.

\medskip

\noindent
Wikipedia Movies Dataset. Kaggle. Retrieved from\\
\url{https://www.kaggle.com/datasets/thedevastator/wikipedia-movie-data-from-1970-2018}

\medskip

\noindent
FAISS library. GitHub repository. Retrieved from\\
